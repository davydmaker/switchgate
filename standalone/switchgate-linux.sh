...
if ! command -v tinyproxy &> /dev/null; then
    ...
    if command -v apt &> /dev/null; then
        echo -e "${YELLOW}Please install it with: sudo apt install tinyproxy${NC}"
    elif command -v dnf &> /dev/null; then
        echo -e "${YELLOW}Please install it with: sudo dnf install tinyproxy${NC}"
    elif command -v pacman &> /dev/null; then
        echo -e "${YELLOW}Please install it with: sudo pacman -S tinyproxy${NC}"
    else
        echo -e "${YELLOW}Please install tinyproxy using your package manager${NC}"
    fi
    exit 1
fi
...
find_tinyproxy_data() {
    local search_dirs=("/usr/share/tinyproxy" "/usr/local/share/tinyproxy" "/etc/tinyproxy")
    ...
    echo "/usr/share/tinyproxy"
}

TINYPROXY_DATA="$(find_tinyproxy_data)"
...
Listen 0.0.0.0
Timeout 600
DefaultErrorFile "${TINYPROXY_DATA}/default.html"
StatFile "${TINYPROXY_DATA}/stats.html"
Logfile "$LOG_FILE"
LogLevel Info
PidFile "$PID_FILE"
MaxClients 100
...
check_port() {
    if ss -tlnp 2>/dev/null | grep -q ":$PROXY_PORT "; then
        ...
        if ss -tlnp 2>/dev/null | grep -q ":$PROXY_PORT "; then
            ...
            fuser -k -9 "$PROXY_PORT/tcp" 2>/dev/null || true
            sleep 1
        fi
    fi
}
